...
public class CardGenerator : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        for(int i=0; i<GameManager.player1.hand.Count; i++) {
            ...
            card_object.GetComponent<CardController>().init(player: 1, hand_index: i, card: card); 
        }
        ...
        for(int i=0; i<GameManager.player2.hand.Count; i++) {
            ...
            card_object.GetComponent<CardController>().init(player: 2, hand_index: i, card: card); 
        }
        //GameManager.turn = 1;
